...
from .tt_operators import identity, hadamard
...
class HADAMARD(Module):
    """Hadamard gate (creates a state of superposition for a single qubit).
...
class IG_Parameter(Module):
    """Involutory generator with an external parameter as input.
    ...
    InvolutoryGenerator
    """
    def __init__(self, involutory_generator, parameter, nqubits, dtype=complex64, device=None):
        super().__init__()
...
class IG_UnitaryParameter(Unitary):
    """A Unitary sub-class that generates a unitary layer with a generator that is involutory (its own inverse).
    ...
    UnaryGatesUnitary
    """
    def __init__(self, nqubits, ncontraq, involutory_generator, parameter, contrsets=None):
        dtype, device = involutory_generator[0].dtype, involutory_generator[0].device
        super().__init__([], nqubits, ncontraq, contrsets=contrsets, dtype=dtype, device=device)
        self._set_gates([IG_Parameter(involutory_generator, parameter, nqubits, dtype=dtype, device=device)])

        
class IG_ParameterWeight(Module):
    """Involutory generator with an external parameter as input.
    ...
    InvolutoryGenerator
    """
    def __init__(self, involutory_generator, parameter, weight, nqubits, dtype=complex64, device=None):
        super().__init__()
        self.theta = parameter
        self.wght = weight
        #self.theta = Parameter(randn(1, device=device))
    ...
    def forward(self):
        ...
        Gate tensor for general forward pass.
        """
        temp_iden, temp_involutory_generator = [self.iden[0]*cos(self.wght * self.theta)] + self.iden[1::], [self.involutory_generator[0]*1j*sin(self.wght * self.theta)] + self.involutory_generator[1::]
        return tt_matrix_sum(temp_iden, temp_involutory_generator).factors


class IG_UnitaryParameterWeight(Unitary):
    """A Unitary sub-class that generates a unitary layer with a generator that is involutory (its own inverse).
    ...
    UnaryGatesUnitary
    """
    def __init__(self, nqubits, ncontraq, involutory_generator, parameter, weight, contrsets=None):
        dtype, device = involutory_generator[0].dtype, involutory_generator[0].device
        super().__init__([], nqubits, ncontraq, contrsets=contrsets, dtype=dtype, device=device)
        self._set_gates([IG_ParameterWeight(involutory_generator, parameter, weight, nqubits, dtype=dtype, device=device)])
